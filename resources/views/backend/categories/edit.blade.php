...
@section('content')
	...
	<section class="page-header">
		<h4 class="text-2xl">{{ __('Editing') }} "<span class="text-info">{{ $category->name }}</span>" {{ __('Category') }}</h4>
		@can('browse-categories')
			<section>
				<a href="{{ route('backend.categories.index') }}" class="btn">
	...
	<form action="{{ route('backend.categories.update', $category->id) }}" method="POST">
		@method('PATCH')
		@include('backend.categories._form', ['btnText' => __('Update')])
	</form>
